...
from util.client_shortcut_handler import (
    hold_mode,
    click_mode
)

speech_recognition_shortcut_test = "'"
...
class KeyboardHandler(QObject):
    ...
    def __init__(self):
        super().__init__()
        self.trigger_hold_mode.connect(self.hold_mode_A)
        self.trigger_click_mode.connect(self.click_mode_A)
        self.bond_shortcut_history_panel()
        # 状态变量：标记按键是否已按下（用于过滤重复 down 事件）
    ...
    def bond_shortcut_history_panel(self):
        if Config.hold_mode:
            keyboard.hook_key(
                ...
                callback=lambda event: self.trigger_hold_mode.emit(event),
                suppress=True  # 传递抑制参数
            )
            print(f"按住模式：快捷键 {speech_recognition_shortcut_test} 已绑定")
        else:
            keyboard.hook_key(
                key=speech_recognition_shortcut_test,
                callback=lambda event: self.trigger_click_mode.emit(event),
                suppress=True
            )
            print(f"单击模式：快捷键 {speech_recognition_shortcut_test} 已绑定")

    def unbond_shortcut(self):
        keyboard.unhook_key(speech_recognition_shortcut_test)


    def hold_mode_A(self, e):
        # 1. 处理按下事件（过滤重复的 down 事件）
        if e.event_type == 'down' and not self.is_pressed:
            self.is_pressed = True  # 标记为已按下，阻止重复处理
            # 输出 KEY_DOWN（按预期只在首次按下和重复触发时执行）
            text = "KEY_DOWN "
            print(f"触发输入：{text}")
