...
class TextLineWidget(QWidget):
    def __init__(self, text, title, is_pinned, parent=None):
        ...
        # 3. 按钮容器（关键：脱离布局，使用绝对定位覆盖在文本上方）
        self.buttons_container = QWidget(self)  # 父对象设为当前行组件
        self.buttons_container.setStyleSheet("background: transparent;")  # 完全透明背景
        self.buttons_container.setWindowFlags(Qt.WindowTransparentForInput)  # 不拦截文本的鼠标事件
        self.buttons_container.hide()  # 初始隐藏
    ...
    def resizeEvent(self, event):
        """绝对定位：按钮容器固定在文本标签的左上角（重叠显示）"""
        super().resizeEvent(event)
        # 获取文本标签在当前组件中的坐标（相对于self的位置）
        label_pos = self.content_label.pos()  # 相对于父组件（TextLineWidget）的位置
        # 按钮容器移动到文本左上角（x=0, y=0 相对于文本标签的位置）
        self.buttons_container.move(label_pos.x(), label_pos.y())
        # 按钮容器大小适应按钮（避免空白区域遮挡文本交互）
