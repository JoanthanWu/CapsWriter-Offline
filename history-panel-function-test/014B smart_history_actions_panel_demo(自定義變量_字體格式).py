...
DEFAULT_CONFIG = {
    ...
    "title": {
        "font_family": "SimHei",
        "font_size": 12,
        "font_bold": False,  # 新增：粗体开关
        "font_italic": False,  # 新增：斜体开关
        "color": [255, 255, 204, 255],  # 标题文字颜色
        "bg_color": [0, 0, 0, 0],       # 透明背景
        "border_color": [0, 0, 0, 0],
        "border_width": 0,
        "min_width": 30,                # 标题最小宽度（避免过窄）
        "max_width": 100,               # 标题最大宽度（防止过长）
        "padding": 5                    # 标题文字左右内边距
    },
    # 内容文本配置
    "content": {
...
class MultiLineElidedLabel(QLabel):
    def __init__(self, text="", parent=None):
        ...
        font.setBold(config["content"]["font_bold"])  # 应用粗体配置
        font.setItalic(config["content"]["font_italic"])  # 应用斜体配置
        self.setFont(font)
    ...
    def setText(self, text):
        ...
        if self.parent() and self.parent().width() > 0:
            ...
            layout_width = config["content"].get("width", 500)
...
class TextLineWidget(QWidget):
    def __init__(self, text, title, is_pinned, is_reviewing, parent=None):
        ...
        font.setBold(config["title"]["font_bold"])  # 应用粗体配置
        font.setItalic(config["title"]["font_italic"])  # 应用斜体配置
        ...
        style_sheet = f"""
            ...
            padding: 0 {config["title"]["padding"]}px;
        """
        ...
        total_width = text_width + 2 * config["title"]["padding"]  # 加上左右内边距

        # 应用 min/max 宽度限制（来自配置）
        min_width = config["title"]["min_width"]
...
class RoundedWidget(QWidget):
    def __init__(self, sentence_group, parent=None, group="unpinned", index=0):
        ...
        # 创建文本行
        for line_config in config["text_lines"]:
        ...
        self.setFixedWidth(config["widget"]["width"])
        self.adjustSize()
    ...
    def _calc_background_size(self):
        ...
        bg_width = config["widget"]["width"]  # 强制使用配置的宽度（700）
