...
2. - [x] 加入對比功能(文字審查功能)
	- 分大小寫
	- 整行對比
3. - [x] 第5个按鈕 : 添加/取消 → history_sanitize_list.txt
4. - [x] pinned & unpinned 的顏色需要区分(并且小组之间需要互相交替,共四组颜色)
5. - ❌ 鍵盤選擇
   	- 上下 選擇文字
   	- 左右 選擇功能
   	- Enter 运行功能
...
10. 優化潜在点
   1. - [!] 面板堆积位置算法
    		...
    		- [x] 新增排序方案一號: `history_panel_arrange_method = 1`
            - 控件（widgets）會依照 列（column）方式 從左到右排列。
            - 每一列的控件由上往下堆疊，直到超出螢幕底部，再換到下一列。
            - 固定組（pinned groups）會優先顯示，非固定組（unpinned groups）隨後顯示。
         - [ ] 排序方案二號: 
            - 改用單個窗口, 囊括所有的 label 文字
	2. - [x] 复用窗口实例
   ...
   2. - [x] 看時機替換窗口關閉的動作`close()` 改爲 `hide()`, 減少窗口建立的次數
         - 如果`pinned_groups or unpinned_groups` 沒有更新的時候使用`hide()`, 如果有更新的時候使用`更新`
         - 关闭操作改为隐藏，保留窗口内存占用但避免重复初始化开销
         - 只有新增的窗口或者不適用的窗口才會進行新建
...
- [x] 問題2. `keyboard库` 无法透过`shift + 双击录音键`来正常开关 面板
   ...
   5. - [ ] !!! `click_mode`translate_needed() 還有一些 bug 可能會導致不需要的翻譯!!!
